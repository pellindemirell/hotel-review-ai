...
public class ReviewAnalysisProcessingServiceTests
{
    ...
    [Fact]
    public async Task ProcessAnalysisAsync_WithAiServiceOffline_ShouldThrowInvalidOperationException()
    {
        // Arrange
        var review = Review.Create(
            "Ahmet Yılmaz",
        ...
        // Act & Assert
        // Outbox pattern retries: AI offline olduğunda exception fırlatılmalı ve job tekrar denenmeli (fake fallback yazılmamalı)
        await Assert.ThrowsAsync<InvalidOperationException>(() => 
            _service.ProcessAnalysisAsync(review, isReanalysis: false, CancellationToken.None));
    }
}
