...
public static class DbSeeder
{
    public static async Task SeedAsync(AppDbContext context)
    {
        ...
        if (context.Database.IsNpgsql())
        {
            ...
            try
            {
                ...
                await SeedUsersAsync(context, hotels);
    ...
    private static async Task SeedUsersAsync(AppDbContext context, List<Hotel> hotels)
    {
        ...
        bool addedNew = false;
        foreach (var (fullName, email, password, role, hotelName, departmentKey) in UserSeedData.Users)
        {
            if (!existingEmailSet.Contains(email))
            {
                var hotel = hotelName is null
                    ? null
                    : hotels.FirstOrDefault(h => string.Equals(h.Name.Trim(), hotelName.Trim(), StringComparison.OrdinalIgnoreCase));

                // Departman, kullanıcının KENDİ otelinden seçilir. Önceden yalnızca ilk otelin
                // departmanları elde tutulduğu için kullanıcı başka bir otelin departmanına bağlanabiliyordu.
                var dept = departmentKey is null || hotel is null
                    ? null
                    : allDepartments.FirstOrDefault(d => d.HotelId == hotel.Id
                        && string.Equals(d.Key, departmentKey, StringComparison.OrdinalIgnoreCase));
