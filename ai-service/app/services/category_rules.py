...
CATEGORY_PHRASES: dict[str, list[tuple[str, float]]] = {
    CAT_CLEANING: [
        ...
        ("kuş yuvası", 7.0), ("kus yuvasi", 7.0), ("ses yalıtımı", 6.5), ("ses yalitimi", 6.5),
    ...
    CAT_FOOD: [
        ...
        ("sıra beklemeniz gerekiyor", 7.5), ("sira beklemeniz gerekiyor", 7.5),
        ("sürekli sıra beklemeniz", 7.5), ("surekli sira beklemeniz", 7.5),
        ("gece gündüz fark etmeden", 6.5), ("gece gunduz fark etmeden", 6.5),
        ...
        ("lezzeti ortalama", 5.0), ("salam tabağı", 6.0), ("salam tabagi", 6.0),
        ("sinekler", 5.0), ("sinek", 4.0),
    ],
    CAT_OTHER: [
    ...
    CAT_STAFF: [
        ...
        ("personel güler yüzlü", 5.5), ("personel guleryuzlu", 5.5),
        ("personel ilgili", 5.0), ("garson ilgili", 5.0), ("çalışan ilgili", 5.0),
        ("personel yardımsever", 5.5), ("garson yardımsever", 5.5),
        ("personel harika", 5.0), ("garson harika", 5.0), ("ekip harika", 5.0),
        ("personel profesyonel", 5.5), ("garson profesyonel", 5.5),
        ("personel güleryüzlü", 5.5), ("personel canayakın", 5.5),
        ("personel gülümsüyor", 5.0), ("garson gülümsüyor", 5.0),
        ("personel memnuniyet", 5.0), ("hizmet kalitesi", 4.5),
        ("ilgi alaka", 5.0), ("güleryüzlü hizmet", 5.5),
        ("personel sorunu", 6.0), ("çalışan sorunu", 5.5),
        ("lakayit davranış", 6.0), ("lakayit yaklaşim", 6.0),
        ("personel suratsız", 6.0), ("personel suratsiz", 6.0),
        ("garson suratsız", 6.0), ("garson suratsiz", 6.0),
        ("servis yavaş", 5.5), ("servis kötü", 5.5), ("servis harika", 5.0),
        ("gümüş tepside servis", 5.0), ("personel koşuyor", 5.0),
    ...
    CAT_TECH: [
        ...
        ("mobil uygulama", 7.0), ("uygulama yavaş", 6.5), ("uygulama yavas", 6.5),
    ],
    CAT_SPA: [
    ...
    CAT_GROUNDS: [
        ("konum güzel", 5.5), ("konum guzel", 5.5), ("konum harika", 5.0), ("konum merkezi", 5.0),
        ("şehir merkezine yakın", 6.0), ("sehir merkezine yakin", 6.0), ("merkeze yürüme", 5.5),
        ("ulaşım kolay", 5.5), ("ulasim kolay", 5.5), ("transfer iyi", 5.0),
        ("otopark sorun", 5.5), ("otopark yetersiz", 5.5), ("park yeri yok", 6.0),
        ("vale hizmeti", 5.0), ("manzara harika", 5.5), ("manzara nefis", 5.0),
        ("bahçe bakımlı", 5.5), ("bahce bakimli", 5.5), ("bahçe güzel", 5.0),
        ("yeşil alan", 5.0), ("yesil alan", 5.0),
        ("güvenlik sorunu", 6.5), ("guvenlik sorunu", 6.5), ("güvenlik zafiyeti", 6.0),
        ("güvensiz hissediyorum", 6.0), ("guvensiz hissediyorum", 6.0),
        ("kamera yok", 5.5), ("kasa sorunu", 5.5),
        ("deniz manzarası", 5.0), ("deniz manzarasi", 5.0),
        ("şehre uzak", 5.5), ("sehire uzak", 5.5), ("her yere uzak", 5.5),
        ("merkez dışı", 5.0), ("merkez disi", 5.0),
        ("shuttle geç", 5.5), ("shuttle yetersiz", 5.5), ("transfer ücretli", 5.0),
        ("çevre düzenlemesi", 5.5), ("cevre duzenlemesi", 5.5), ("peyzaj güzel", 5.0),
    ],
    "FINANCE_TEMP": [
...
CATEGORY_KEYWORD_WEIGHTS: dict[str, dict[str, float]] = {
    ...
    CAT_STAFF: {
        "garson": 3.5, "personel": 3.0, "çalışan": 2.5, "çalışanlar": 3.0, "calisanlar": 3.0,
        "kaba": 2.5, "ilgisiz": 2.5, "kibar": 3.5, "kibardı": 3.5, "kibardi": 3.5,
        "saygısız": 2.5, "yavaş": 1.5, "hostes": 2.5, "resepsiyonist": 2.0,
        "güleryüzlü": 3.0, "guleryuzlu": 3.0, "güler": 2.5, "guler": 2.5,
        "yardımsever": 2.5, "yardimsever": 2.5, "profesyonel": 2.5,
        "ilgili": 2.0, "canayakın": 2.5, "canayakin": 2.5, "sicakkanli": 2.5, "sıcakkanlı": 2.5,
    ...
    CAT_SPA: {
        ...
        "sezlong": 2.5, "şenzlog": 2.5, "animasyon": 3.0, "wellness": 3.0, "sauna": 2.5, "jakuzi": 2.5, "fitness": 2.0,
        "etkinlik": 3.0, "etkinlikler": 3.0, "aquapark": 3.5, "aquaprk": 3.5, "kaydirak": 2.5,
        "tenis": 2.5, "cakil": 2.0, "çakıl": 2.0,
    },
    CAT_GROUNDS: {
        "konum": 3.0, "lokasyon": 3.0, "merkez": 2.5, "merkezi": 2.5,
        "ulasim": 3.0, "ulaşım": 3.0, "transfer": 3.5, "shuttle": 3.0,
        "taksi": 2.5, "dolmus": 2.5, "dolmuş": 2.5, "otobus": 2.0,
        "otopark": 3.5, "vale": 3.0, "park": 2.5,
        "guvenlik": 3.0, "güvenlik": 3.0, "kamera": 2.5, "kasa": 2.5, "bekci": 2.5,
        "cevre": 3.0, "çevre": 3.0, "bahce": 2.5, "bahçe": 2.5,
        "peyzaj": 2.5, "yesil": 2.0, "yeşil": 2.0,
        "manzara": 3.0, "deniz": 2.0, "kumsal": 2.5, "sahil": 2.5,
        "uzak": 2.0, "yakin": 2.0, "yakın": 2.0,
    },
    "FINANCE_TEMP": {
...
_finance_dict = CATEGORY_KEYWORD_WEIGHTS.pop("FINANCE_TEMP", {})
CATEGORY_KEYWORD_WEIGHTS[CAT_RECEPTION] = {**_reception_dict, **_finance_dict}
...
def _apply_disambiguation(cleaned: str, tokens: list[str], scores: dict[str, float]) -> None:
    ...
    queue_drink = any(w in cleaned for w in ("sira", "sıra", "kuyruk", "bekleniyor", "dakika", "beklemeniz", "beklemek")) and any(
        w in cleaned for w in ("icecek", "içecek", "bar", "tekila", "alkol", "limonata", "alakart", "alakarta", "mojita", "restoran", "bufe", "büfe")
    )
    # Bare queue/sıra (no bar/pool/check-in/kids) → F&B; never Teknik/HVAC
    bare_queue = any(w in cleaned for w in ("sira", "sıra", "kuyruk", "beklemeniz", "beklemek zorunda", "surekli sira", "sürekli sıra"))
    pool_queue = bare_queue and any(w in cleaned for w in ("sezlong", "şezlong", "havuz", "aquapark", "kaydirak", "kaydırak", "plaj", "cocuk", "çocuk"))
    fo_queue = bare_queue and any(w in cleaned for w in ("check-in", "checkin", "check-out", "checkout", "resepsiyon", "lobi", "asansor", "asansör"))
    hvac_queue_ctx = any(w in cleaned for w in ("klima", "sogut", "soğut", "serinlet", "termostat", "hvac", "isitma", "ısıtma"))
    if bare_queue and not pool_queue and not fo_queue and not hvac_queue_ctx:
        scores[CAT_FOOD] += 9.0
        scores[CAT_TECH] -= 9.0
        scores[CAT_SPA] -= 4.0
    # "sıra beklemeniz" alone is NOT enough for F&B when kids/kaydırak/havuz present
    if queue_drink:
        scores[CAT_FOOD] += 7.0
        scores[CAT_SPA] -= 5.0
        scores[CAT_TECH] -= 6.0
        scores[CAT_RECEPTION] -= 2.0
    elif any(w in cleaned for w in ("15 20 dakika", "30-45 dakika", "30 45 dakika", "sira bekleniyor", "sıra bekleniyor", "sira beklemeniz", "sıra beklemeniz")):
        if pool_queue:
            scores[CAT_SPA] += 6.0
            scores[CAT_FOOD] -= 6.0
            scores[CAT_TECH] -= 5.0
        elif not fo_queue and not hvac_queue_ctx:
            scores[CAT_FOOD] += 7.0
            scores[CAT_SPA] -= 5.0
            scores[CAT_TECH] -= 6.0
            scores[CAT_RECEPTION] -= 2.0

    # Kids overcrowding / aquapark wait → Rekreasyon (not F&B)
    if any(w in cleaned for w in ("cocuk sayisi", "çocuk sayısı", "cocuklar", "çocuklar")) and any(
        w in cleaned for w in ("sira", "sıra", "kuyruk", "bekle", "asiri", "aşırı", "fazla", "kalabal")
    ):
        scores[CAT_SPA] += 7.0
        scores[CAT_FOOD] -= 8.0
        scores[CAT_TECH] -= 4.0

    # Property walk / size — not F&B queue, not room HK
    if any(w in cleaned for w in ("yurumek", "yürümek", "yurumeyi", "yürümeyi", "uzun suruyor", "uzun sürüyor")) and any(
        w in cleaned for w in ("havuz", "deniz", "kaydirak", "kaydırak", "plaj")
    ):
        if not any(w in cleaned for w in ("sira", "sıra", "kuyruk")):
            scores[CAT_OTHER] += 5.0
            scores[CAT_FOOD] -= 5.0
            scores[CAT_CLEANING] -= 3.0
    if any(w in cleaned for w in ("otel cok buyuk", "otel çok büyük", "yurumeyi sevmeyen", "yürümeyi sevmeyen")):
        scores[CAT_OTHER] += 5.0
        scores[CAT_CLEANING] -= 4.0

    # Common-area capacity — not room cleanliness
    if any(w in cleaned for w in ("oturabilecegi", "oturabileceği", "yeterli alan yok", "yeterli yer yok", "ortak alan")):
        scores[CAT_OTHER] += 5.0
        scores[CAT_CLEANING] -= 6.0
        scores[CAT_FOOD] -= 2.0

    # Bar staffing count → F&B (not generic staff attitude)
    if any(w in cleaned for w in ("barlardaki calisan", "barlardaki çalışan", "bar calisan", "bar çalışan")) and any(
        w in cleaned for w in ("az", "yetersiz", "sayi", "sayı")
    ):
        scores[CAT_FOOD] += 6.0
        scores[CAT_STAFF] -= 4.0

    # Alakart / portakal suyu ek ücret → F&B
    if any(w in cleaned for w in ("alakart", "alakarta", "alacarte", "portakal suyu", "sikma portakal", "sıkma portakal")):
        scores[CAT_FOOD] += 5.0
        scores[CAT_TECH] -= 4.0
        if any(w in cleaned for w in ("parali", "paralı", "ucret", "ücret", "gereksiz")):
            scores[CAT_FOOD] += 3.0

    # Yer tutma / şezlong kapasitesi → Rekreasyon (not Teknik)
    if any(w in cleaned for w in ("yer tutma", "yer bulamaz", "sezlong", "şezlong", "erken gitmezseniz")):
        scores[CAT_SPA] += 5.0
        scores[CAT_TECH] -= 6.0
        scores[CAT_OTHER] += 1.0

    # Housekeeping mahremiyet / temizlik çalışanı odaya girdi
    if any(w in cleaned for w in ("temizlik calisan", "temizlik çalışan", "odaya girdi", "cleaning cleaning", "hosuma gitmeyen", "hoşuma gitmeyen")):
        scores[CAT_CLEANING] += 6.0
        scores[CAT_TECH] -= 6.0
    ...
    # Personel davranışı: garson/personel/çalışan + olumsuz davranış → Personel
    staff_neg = {"kaba", "ilgisiz", "saygısız", "yavaş", "küfür", "bağırdı", "suratsız", "suratsiz", "lakayit", "kötü muamele"}
    if any(w in tokens or w in cleaned for w in ("garson", "personel", "personelin", "çalışan", "calisan", "barmen", "hostes", "animatör")):
        if any(b in tokens or b in cleaned for b in staff_neg):
            if not any(p in cleaned for p in ("yemek lezzetli", "yemek harika", "yemek mükemmel")):
                scores[CAT_STAFF] += 5.0
                scores[CAT_FOOD] -= 2.0
                scores[CAT_RECEPTION] -= 1.0
        # Personel + olumlu davranış → Personel (yemek övgüsü yoksa)
        staff_pos = {"kibar", "güler", "guleryuz", "güleryüzlü", "yardımsever", "yardimsever", "profesyonel", "ilgili", "gülümsüyor", "canayakın", "canayakin", "sicakkanli", "sıcakkanlı"}
        if any(p in tokens or p in cleaned for p in staff_pos):
            if not any(f in cleaned for f in ("yemek lezzetli", "yemek harika", "yemek mükemmel", "yemekler harika", "kahvaltı harika")):
                scores[CAT_STAFF] += 4.0
                if "garson" not in tokens and "barmen" not in tokens:
                    scores[CAT_FOOD] -= 1.5
    ...
    staff_behavior_words = ("güler yüz", "ilgisiz", "kaba", "harika", "ilgili", "tutum", "davranış")
    if "animatör" in tokens and any(b in tokens or b in cleaned for b in staff_behavior_words):
        scores[CAT_STAFF] += 3.0
        scores[CAT_SPA] -= 1.0
    ...
    # Çevre/Güvenlik/Ulaşım: konum/manzara/otopark → CAT_GROUNDS
    if any(w in tokens for w in ("konum", "lokasyon", "manzara")):
        if any(p in cleaned for p in ("güzel", "guzel", "harika", "mükemmel", "mukemmel", "nefis", "merkezi", "yakın", "yakin")):
            scores[CAT_GROUNDS] += 5.0
            scores[CAT_OTHER] -= 2.0
        elif any(p in cleaned for p in ("kötü", "kotu", "berbat", "uzak", "sorun")):
            scores[CAT_GROUNDS] += 5.0
            scores[CAT_STAFF] -= 1.0
    if "otopark" in cleaned or any(w in cleaned for w in ("park sorunu", "park yeri yok", "vale")):
        scores[CAT_GROUNDS] += 4.0
        scores[CAT_TECH] -= 1.0
    if any(w in tokens for w in ("transfer", "shuttle", "ulasim", "ulaşım")):
        scores[CAT_GROUNDS] += 4.0
        scores[CAT_RECEPTION] -= 1.0
    if any(w in tokens for w in ("guvenlik", "güvenlik", "kamera", "kasa", "bekci")):
        scores[CAT_GROUNDS] += 4.0
    if any(w in cleaned for w in ("bahçe güzel", "bahce guzel", "bahçe bakımlı", "bahce bakimli", "yeşil alan", "yesil alan", "peyzaj")):
        scores[CAT_GROUNDS] += 4.0
        scores[CAT_SPA] -= 1.0
    if "deniz" in tokens and any(w in cleaned for w in ("manzara", "güzel", "guzel", "harika")):
        if "havuz" not in tokens:
            scores[CAT_GROUNDS] += 3.0
            scores[CAT_SPA] -= 1.0
