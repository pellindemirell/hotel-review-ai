...
def _pick_best(rules: list[_SubRule], cleaned: str, sentiment: str) -> Optional[str]:
    ...
    for rule in rules:
        ...
        if match_count == 0:
            continue
        # Hard mismatch: Positive praise must not get Negative ops advice (and vice versa).
        # Neutral must not inherit Negative cancel/complaint templates from topic-only cues.
        if rule.sentiment_hint and rule.sentiment_hint != sentiment:
            continue
        bonus = 0
...
FOOD_RULES = [
    _SubRule(
        ("hamam böceği", "hamam bocegi", "hamambocegi", "böcek", "bocek", "haşere", "hasere",
         "boceginin", "böceğinin", "yemek salonunda"),
        "F&B ve haşere kontrol ekibi yemek salonu/mutfak için acil pest control ve hijyen denetimi başlatmalı; olay kaydı açılıp misafire özür ve telafi süreci uygulanmalı. Spa hizmeti aksiyonu uygulanmaz.",
        weight=16,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("sindirim", "zehirlen", "gıda güven", "gida guven", "mide bulant", "ishal", "yemeklerden kaynakli", "yemeklerden kaynaklı"),
        "F&B müdürü gıda güvenliği ve mutfak hijyen protokolünü acilen denetlemeli; soğuk zincir, pişirme sıcaklığı ve numune kayıtları kontrol edilmeli; gıda kaynaklı rahatsızlık için misafir ilişkileri süreci başlatılmalı.",
        weight=15,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("alerjiniz", "alerji", "allerji", "alerjen", "allergen"),
        "F&B alerjen bilgilendirme protokolü standart olarak uygulanmalı; girişte alerji sorusu olumlu bir hizmet adımıdır, tadım/lezzet kalite aksiyonu gerektirmez.",
        weight=9,
        sentiment_hint="Neutral",
    ),
    _SubRule(
        ("paralı", "parali", "ücretli", "ucretli", "ekstra ücret", "ekstra ucret",
         "ücret isteniyor", "ucret isteniyor", "ücretli olması", "portakal suyu", "meyve suyu"),
        "F&B müdürü her şey dahil kapsamındaki ücretli içecek/ameniti politikasını gözden geçirmeli; sıkma portakal suyu, kahve vb. ekstra ücret kalemleri misafir beklentisiyle uyumlu hale getirilmeli veya şeffaf şekilde duyurulmalı.",
        weight=14,
        sentiment_hint="Negative",
    ),
    _SubRule(
...
CLEANING_RULES = [
    _SubRule(
        ("mahremiyet", "odaya girdi", "kapıyı çalmadan", "kapiyi calmadan", "sorry", "cleaning cleaning",
         "hoşuma gitmeyen", "hosuma gitmeyen", "hoş değildi", "hos degildi", "temizlik çalışan", "temizlik calisan"),
        "Kat hizmetleri müdürü oda mahremiyeti protokolünü gözden geçirmeli; kapı çalma/izin ve 'cleaning' duyurusu standartı zorunlu hale getirilmeli, ihlal eğitimi planlanmalı.",
        weight=14,
        sentiment_hint="Negative",
    ),
    _SubRule(
    ...
    _SubRule(
        ("tertemiz", "pırıl", "pırıl pırıl", "hijyenik", "çok temizdi", "cok temizdi", "oda temizdi"),
...
SPA_RULES = [
    _SubRule(
        ("hamam böceği", "hamam bocegi", "hamambocegi", "böcek", "bocek", "haşere", "hasere"),
        "F&B/housekeeping haşere kontrolü ve yemek alanı hijyen denetimi acilen başlatılmalı; spa hizmet aksiyonu uygulanmaz.",
        weight=20,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("deniz", "denizi", "sahil", "sahile", "plaj", "su spor", "dalgalı", "dalgali", "bulanık", "bulanik"),
        "Plaj/deniz operasyonu su sporları yerleşimini ve sahil mesafesini gözden geçirmeli; dalgalı/bulanık deniz koşulları için misafir bilgilendirmesi yapılmalı. Havuz derinliği veya spa aksiyonu değil.",
        weight=14,
        sentiment_hint="Negative",
    ),
    _SubRule(
    ...
    _SubRule(
        ("masaj", "spa ", " sauna", "wellness", "türk hamamı", "turk hamami"),
    ...
    _SubRule(
        ("keyif ald", "fazlasıyla keyif", "fazlasiyla keyif", "aşırı keyifli", "asiri keyifli",
         "keyifli", "başarılıydı", "basariliydi", "eğlenceyle", "eglenceyle"),
        "Animasyon ve etkinlik ekibine olumlu geri bildirim iletilmeli; akşam programı ve eğlence standardı korunup takdir notu paylaşılmalı.",
        weight=12,
        sentiment_hint="Positive",
    ),
    _SubRule(
        ("kaldırılmış", "kaldirilmis", "iptal", "bayram nedeniyle", "masa tenisi",
         "aktiviteler kaldır", "aktivite kaldır", "program iptal"),
...
STAFF_RULES = [
    _SubRule(
        ("personel sayısı", "personel sayisi", "personel sayısında", "personel sayisinda",
         "yetersizlik", "personel az", "personel eksik", "kadro yetersiz", "bayram"),
        "İnsan kaynakları ve operasyon müdürü bayram/yoğun dönem personel planlamasını gözden geçirmeli; kritik departmanlarda geçici kadro ve vardiya takviyesi yapılmalı.",
        weight=14,
        sentiment_hint="Negative",
    ),
    _SubRule(
...
_DOMINANT_OPS_RULES: list[_SubRule] = [
    _SubRule(
        ("mahremiyet", "odaya girdi", "sorry", "cleaning cleaning", "hoşuma gitmeyen", "hosuma gitmeyen"),
        "Kat hizmetleri müdürü oda mahremiyeti protokolünü gözden geçirmeli; kapı çalma/izin ve duyuru standardı zorunlu hale getirilmeli.",
        weight=22,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("paralı", "parali", "ücretli olması", "ucretli olmasi", "ekstra ücret", "portakal suyu paralı", "portakal suyunun paralı"),
        "F&B müdürü her şey dahil kapsamındaki ücretli içecek/ameniti politikasını gözden geçirmeli; ekstra ücret kalemleri şeffaf duyurulmalı veya pakete dahil edilmeli.",
        weight=23,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("kaydırak", "kaydirak", "çocuk sayısı", "cocuk sayisi", "aquapark", "aquaprk"),
        "Rekreasyon/havuz operasyonu aquapark ve çocuk yoğunluğuna göre kapasite ve sıra yönetimini planlamalı; kaydırak/havuz bekleme süreleri ölçülmeli.",
        weight=21,
        sentiment_hint="Negative",
    ),
    _SubRule(
        ("alakart", "bar", "içecek", "icecek", "büfe", "bufe", "restoran", "pankek"),
    ...
    _SubRule(
        ("sıra", "sira", "kuyruk", "dakika bek", "bekleniyor", "alınamıyor"),
        "Operasyon yönetimi ilgili servis noktasındaki sıra/bekleme süresini ölçmeli; yoğunluk planı ve personel dağılımı güncellenmeli.",
        weight=15,
        sentiment_hint="Negative",
    ),
    _SubRule(
